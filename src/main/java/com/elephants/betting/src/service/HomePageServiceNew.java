...
//                    .matches(scheduler.getMatchesToShowOnHomePage().stream().sorted((a,b)-> Boolean.compare(b.getIsLiveMatch(), a.getIsLiveMatch())).toList())
                    .matches(scheduler.getMatchesToShowOnHomePage().stream().filter(cricketMatches -> Boolean.TRUE.equals(cricketMatches.getIsLiveMatch()) && Boolean.FALSE.equals(cricketMatches.isTestMatch())).toList())
                    .build();
